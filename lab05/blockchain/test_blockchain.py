...
previous_hash = previous_block.hash  # Sửa lỗi ở đây
my_blockchain.add_transaction('Genesis', 'Miner', 1)
new_block = my_blockchain.create_block(new_proof, previous_hash)

# Displaying the blockchain
for block in my_blockchain.chain:
    print(f"Block #{block.index}")  # Kiểm tra xem `index` có tồn tại không
    print("Timestamp:", block.timestamp)
    print("Transactions:", block.transactions)  # Sửa lỗi tên thuộc tính
